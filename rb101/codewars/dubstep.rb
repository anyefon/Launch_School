...
first word of the song (the number may be zero), after the last word (the number may be zero), and between
 words (at least one between any pair of neighbouring words), and then the boy glues together all the words, 
...
-string
output:
-string with all the 'WUB' substrings removed

algorithms:
-substitute all instances of "WUB" with an empty string
-split string up into substrings
-join substrings with spaces between
=end
