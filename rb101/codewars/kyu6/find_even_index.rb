...
output:
-lowest index of array where side left of index equals side right of index (addition)
rules:
...
- for each value in arr
  -push current_index to indices if sum from 0..current_index - 1 equals sum from current_index + 1 to
   array end
- return lowest index in indices array or -1 if indices array is empty
=end
...
def find_even_index(arr)
  ...
  arr.each_with_index do |num, index|
    numbers << index if arr[0, index].sum == arr[index + 1, arr.length - 1].sum
  end
  numbers.empty? ? -1 : numbers.min
end